                ...
                <li class="nav-profile">
                    <div class="media">
                        <div class="user-icon"><img class="img-fluid rounded-50"
                                                    src="{{asset(!empty(Auth::user()->profile)? $imgpath.Auth::user()->profile : $imgpath.Auth::user()->profile.'avatar.png')}}"
                                                    {{-- src="{{asset(!empty($users->profile)? $imgpath.$users->profile : $imgpath.'avatar.png')}}" --}}
                                                    {{-- src="{{ route('images.show',$imgpath, (!empty($users->avatar)? $users->avatar : 'avatar.png') ) }}" --}}
                                                    {{-- src="{{route('images.show',(!empty($users->profile)? $users->profile : 'avatar.png'))}}" --}}
                                                    alt="logo"></div>
